...
{
    [Export]
    public class CreateMemberViewModel : GenericViewModelbase
    {
        #region [ Constructors ]
        ...
        #region [ Fields ]
        public ObservableCollection<MembershipTierDto> MembershipTierList { get; set; } = new ObservableCollection<MembershipTierDto>();
        ...
        private void OnSubmitCommand()
        {
            ...
            try
            {
                ...
                {
                    Member = new ConnectMemberDto
                    {
                        Name = Name,
                        LastName = LastName,
                        BirthDay = BirthDay,
                        MemberCode = Guid.NewGuid().ToString(),
                        PhoneNumber = Mobile,
                        EmailId = Email,
                ...
                if(response == null)
                {
                    NotifyPopupService.Notify("Member", "Member Creation failed", true, new TimeSpan(0, 0, 5));
                }
                else
                {
                    ...
                    {
                        NotifyPopupService.Notify("Member", "Member Creation Successful", true, new TimeSpan(0, 0, 5));
                        ...
                        var searchInput = new SearchUserInput
                        {
                            Filter = input.Member.Name,
                            TenantId = SettingService.ProgramSettings.SyncTenantId
                        };
                        var searchResponse = TokenService.GetResponse(ConnectRequests.SEARCHMEMBER, searchInput);
                        if (searchResponse != null)
                        {
                            var output = JsonConvert.DeserializeObject<ConnectMemberListDto>(searchResponse.ToString());
                            var mb = output.Items.ToList().FirstOrDefault(x => x.Id == user.Id);
                            if (mb != null)
                            {
                                MemberManagementViewModel.Members.Add(mb);
                    ...
                    {
                        NotifyPopupService.Notify("Member", user != null ? user.ErrorMessage : "Member Creation failed",
                            true, new TimeSpan(0, 0, 5));
                    }
                }
