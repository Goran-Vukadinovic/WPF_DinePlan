...
using DinePlan.Modules.EntityModule.Properties;
using DinePlan.Presentation.Common;
using DinePlan.Presentation.Controls.ListViewLM;
using System;
...
namespace DinePlan.Modules.EntityModule
{
    ...
    public partial class EntitySearchView : UserControl
    {
        ...
        private void viewModel_SelectedAccountTypeChanged(object sender, EventArgs e)
        {
            var gridView = MainListView.View as GridView;
            if (ViewModel != null && gridView != null)
            {
                gridView.Columns.Where(x => x != gridView.Columns.First()).ToList()
                    .ForEach(x => gridView.Columns.Remove(x));
                if (ViewModel.SelectedEntityType != null)
                    ViewModel.SelectedEntityType.EntityCustomFields.Where(x => !x.Hidden).Select(CreateColumn).ToList()
                        .ForEach(x => gridView.Columns.Add(x));
        ...
        private static GridViewColumn CreateColumn(EntityCustomField customField)
        {
            ...
            fef.SetBinding(TextBlock.TextProperty,
                new Binding("[" + customField.Name + "]") { StringFormat = customField.EditingFormat });
        ...
        private void UserControl_Unloaded(object sender, RoutedEventArgs e)
        {
            if (ViewModel.IsKeyboardVisible)
            {
                Settings.Default.KeyboardHeight = KeyboardRow.Height.Value;
                Settings.Default.ContentHeight = ContentRow.Height.Value;
