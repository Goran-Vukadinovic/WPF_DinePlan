...
namespace DinePlan.Modules.AccountModule
{
    ...
    public partial class BatchDocumentCreatorView : UserControl
    {
        ...
        private void viewModel_OnUpdate(object sender, EventArgs e)
        {
            var d = DataContext as BatchDocumentCreatorViewModel;
            MainDataGrid.Columns.Where(x => x.MinWidth == 59).ForEach(x => MainDataGrid.Columns.Remove(x));
            if (d != null)
            {
                var i = 2;
                foreach (var accountType in d.GetNeededAccountTypes())
                {
                    var accountNameBinding = new Binding(string.Format("[{0}].AccountName", accountType.Id))
            ...
            var dataView = CollectionViewSource.GetDefaultView(MainDataGrid.ItemsSource);
